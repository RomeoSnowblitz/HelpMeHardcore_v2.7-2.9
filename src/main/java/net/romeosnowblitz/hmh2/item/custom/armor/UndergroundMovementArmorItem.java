...
public class UndergroundMovementArmorItem extends ArmorItem {
    public UndergroundMovementArmorItem(ArmorMaterial material, Type type, Settings settings) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (entity instanceof PlayerEntity player) {
            ...
            if (numberOfArmorPieces(player)>0 && attackingEntity != null && attackingEntity != player) {t++;
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_HEALTH_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_NOURISHMENT_ITEM)) {
                    armorStrengthHeal(player, 3);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_HEALTH_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_NOURISHMENT_ITEM) ||
                attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_MOVEMENT_ITEM)) {
                    armorStrengthHeal(player, 2);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_HEALTH_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_NOURISHMENT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_MOVEMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_SIGHT_ITEM)) {
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_NOURISHMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_MOVEMENT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_SIGHT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_CHARISMA_ITEM)) {
                    armorWeaknessAttack(world, player, 1);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_MOVEMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_SIGHT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_CHARISMA_ITEM)) {
                    armorWeaknessAttack(world, player, 2);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_SIGHT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_CHARISMA_ITEM)) {
                    armorWeaknessAttack(world, player, 3);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_CHARISMA_ITEM)) {
    ...
    public int numberOfArmorPieces(PlayerEntity player){
        int number = 0;
        if (player.getEquippedStack(EquipmentSlot.HEAD).isIn(ModItemTags.UNDERGROUND_MOVEMENT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.CHEST).isIn(ModItemTags.UNDERGROUND_MOVEMENT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.LEGS).isIn(ModItemTags.UNDERGROUND_MOVEMENT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.FEET).isIn(ModItemTags.UNDERGROUND_MOVEMENT_ARMOR)) {number++;}
        return number;
    }
