...
public class UndergroundHealthArmorItem extends ArmorItem {
    public UndergroundHealthArmorItem(ArmorMaterial material, Type type, Settings settings) {
        ...
        if (entity instanceof PlayerEntity player) {
            ...
            if (numberOfArmorPieces(player)>0 && attackingEntity != null) {t++;
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_HEALTH_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_MOVEMENT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_NOURISHMENT_ITEM)) {
                    armorWeaknessAttack(world, player, 2);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_MOVEMENT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_NOURISHMENT_ITEM)) {
    ...
    public void armorStrengthHeal(PlayerEntity player, int heal){
        if(24/numberOfArmorPieces(player) == t){t=0;
            player.heal(heal+numberOfArmorPieces(player));
        }
    }
    ...
    public int numberOfArmorPieces(PlayerEntity player){
        int number = 0;
        if (player.getEquippedStack(EquipmentSlot.HEAD).isIn(ModItemTags.UNDERGROUND_HEALTH_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.CHEST).isIn(ModItemTags.UNDERGROUND_HEALTH_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.LEGS).isIn(ModItemTags.UNDERGROUND_HEALTH_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.FEET).isIn(ModItemTags.UNDERGROUND_HEALTH_ARMOR)) {number++;}
        return number;
    }
