...
public class ArialSightArmorItem extends ArmorItem {
    public ArialSightArmorItem(ArmorMaterial material, Type type, Settings settings) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (entity instanceof PlayerEntity player) {
            ...
            if (numberOfArmorPieces(player)>0 && attackingEntity != null) {t++;
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_NOURISHMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_MOVEMENT_ITEM)) {
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_NOURISHMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_MOVEMENT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_SIGHT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_CHARISMA_ITEM)) {
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_MOVEMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_SIGHT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_CHARISMA_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_HEALTH_ITEM)) {
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_SIGHT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_CHARISMA_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_HEALTH_ITEM)) {
                    armorWeaknessAttack(world, player, 2);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_CHARISMA_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_HEALTH_ITEM)) {
    ...
    public int numberOfArmorPieces(PlayerEntity player){
        int number = 0;
        if (player.getEquippedStack(EquipmentSlot.HEAD).isIn(ModItemTags.ARIAL_SIGHT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.CHEST).isIn(ModItemTags.ARIAL_SIGHT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.LEGS).isIn(ModItemTags.ARIAL_SIGHT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.FEET).isIn(ModItemTags.ARIAL_SIGHT_ARMOR)) {number++;}
        return number;
    }
