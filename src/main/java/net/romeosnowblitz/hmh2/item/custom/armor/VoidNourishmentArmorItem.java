...
public class VoidNourishmentArmorItem extends ArmorItem {
    public VoidNourishmentArmorItem(ArmorMaterial material, Type type, Settings settings) {
        ...
        if (entity instanceof PlayerEntity player) {
            ...
            if (numberOfArmorPieces(player)>0 && attackingEntity != null && attackingEntity != player) {t++;
                ...
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_CHARISMA_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_SIGHT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_NOURISHMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.UNDERGROUND_HEALTH_ITEM)) {
                    armorStrengthHeal(player, 1);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_SIGHT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_NOURISHMENT_ITEM) ||
                        attackingEntity.getMainHandStack().isIn(ModItemTags.VOID_HEALTH_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.NETHER_MOVEMENT_ITEM)) {
                    armorWeaknessAttack(world, player, 1);
                }
                if (attackingEntity.getMainHandStack().isIn(ModItemTags.ARIAL_NOURISHMENT_ITEM) || attackingEntity.getMainHandStack().isIn(ModItemTags.AQUATIC_HEALTH_ITEM) ||
    ...
    public int numberOfArmorPieces(PlayerEntity player){
        int number = 0;
        if (player.getEquippedStack(EquipmentSlot.HEAD).isIn(ModItemTags.VOID_NOURISHMENT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.CHEST).isIn(ModItemTags.VOID_NOURISHMENT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.LEGS).isIn(ModItemTags.VOID_NOURISHMENT_ARMOR)) {number++;}
        if (player.getEquippedStack(EquipmentSlot.FEET).isIn(ModItemTags.VOID_NOURISHMENT_ARMOR)) {number++;}
        return number;
    }
