...
public class ModPistonBlockEntity extends BlockEntity {
    ...
    public ModPistonBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MOD_PISTON, pos, state);
    }

    public ModPistonBlockEntity(BlockPos pos, BlockState state, BlockState pushedBlock, Direction facing, boolean extending, boolean source) {
        this(pos, state);
    ...
    private BlockState getHeadBlockState() {
        if (!this.isExtending() && this.isSource() && this.pushedBlock.getBlock() instanceof ModPistonBlock) {
            return (BlockState)((BlockState)((BlockState) ModBlocks.MOD_PISTON_HEAD.getDefaultState().with(ModPistonHeadBlock.SHORT, this.progress > 0.25f)).with(ModPistonHeadBlock.TYPE, this.pushedBlock.isOf(ModBlocks.GLUEY_PISTON) ? ModPistonType.GLUEY : ModPistonType.DEFAULT)).with(ModPistonHeadBlock.FACING, this.pushedBlock.get(ModPistonBlock.FACING));
        }
        return this.pushedBlock;
    }

    private static void pushEntities(World world, BlockPos pos, float f, ModPistonBlockEntity blockEntity) {
        ...
        Box box = ModPistonBlockEntity.offsetHeadBox(pos, voxelShape.getBoundingBox(), blockEntity);
        ...
        for (Entity entity : list) {
            ...
            while (!(!iterator.hasNext() || (box3 = Boxes.stretch(ModPistonBlockEntity.offsetHeadBox(pos, box2 = iterator.next(), blockEntity), direction, d)).intersects(box4 = entity.getBoundingBox()) && (i = Math.max(i, ModPistonBlockEntity.getIntersectionSize(box3, direction, box4))) >= d)) {
            }
            if (i <= 0.0) continue;
            i = Math.min(i, d) + 0.01;
    ...
    private static void moveEntitiesInHoneyBlock(World world, BlockPos pos, float f, ModPistonBlockEntity blockEntity) {
        ...
        Box box = ModPistonBlockEntity.offsetHeadBox(pos, new Box(0.0, d, 0.0, 1.0, 1.5000000999999998, 1.0), blockEntity);
        double e = f - blockEntity.progress;
        List<Entity> list = world.getOtherEntities(null, box, entity -> ModPistonBlockEntity.canMoveEntity(box, entity));
    ...
    private static Box offsetHeadBox(BlockPos pos, Box box, ModPistonBlockEntity blockEntity) {
    ...
    private static void push(BlockPos pos, Entity entity, Direction direction, double amount) {
        ...
        if (box.intersects(box2 = VoxelShapes.fullCube().getBoundingBox().offset(pos)) && Math.abs((d = ModPistonBlockEntity.getIntersectionSize(box2, direction2 = direction.getOpposite(), box) + 0.01) - (e = ModPistonBlockEntity.getIntersectionSize(box2, direction2, box.intersection(box2)) + 0.01)) < 0.01) {
            d = Math.min(d, amount) + 0.01;
            ModPistonBlockEntity.moveEntity(direction, entity, d, direction2);
        }
    }
    ...
    public void finish() {
        if (this.world != null && (this.lastProgress < 1.0f || this.world.isClient)) {
            ...
            this.markRemoved();
            if (this.world.getBlockState(this.pos).isOf(ModBlocks.MOD_MOVING_PISTON)) {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, ModPistonBlockEntity blockEntity) {
        ...
        if (blockEntity.lastProgress >= 1.0f) {
            ...
            if (world.getBlockState(pos).isOf(ModBlocks.MOD_MOVING_PISTON)) {
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        this.pushedBlock = NbtHelper.toBlockState((RegistryEntryLookup)registryEntryLookup, nbt.getCompound("blockState"));
    ...
    public VoxelShape getCollisionShape(BlockView world, BlockPos pos) {
        VoxelShape voxelShape = !this.extending && this.source && this.pushedBlock.getBlock() instanceof ModPistonBlock ? ((BlockState)this.pushedBlock.with(ModPistonBlock.EXTENDED, true)).getCollisionShape(world, pos) : VoxelShapes.empty();
        Direction direction = field_12205.get();
        if ((double)this.progress < 1.0 && direction == this.getMovementDirection()) {
            return voxelShape;
        }
        BlockState blockState = this.isSource() ? (BlockState)((BlockState)ModBlocks.MOD_PISTON_HEAD.getDefaultState().with(ModPistonHeadBlock.FACING, this.facing)).with(ModPistonHeadBlock.SHORT, this.extending != 1.0f - this.progress < 0.25f) : this.pushedBlock;
