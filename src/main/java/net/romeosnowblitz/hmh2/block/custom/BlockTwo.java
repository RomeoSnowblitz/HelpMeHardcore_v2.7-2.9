...
public class BlockTwo extends Block {
    ...
    public BlockTwo(Settings settings) {
        super(settings);
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (world.isAir(pos.up())) {
            ...
            if (i < 3) {
                ...
                if (j == 15) {
                    world.setBlockState(pos.up(), ModBlocks.BLOCK_THREE.getDefaultState());
    ...
    protected void update(World world, BlockPos pos) {
        if (this.absorbSouls(world, pos)) {
            world.setBlockState(pos, ModBlocks.BLOCK_TWO.getDefaultState(), Block.NOTIFY_LISTENERS);
    ...
    private boolean absorbSouls(World world, BlockPos pos) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                world.setBlockState(blockPos2, ModBlocks.BLOCK_THREE.getDefaultState(), Block.NOTIFY_ALL);
                ++i;
                if (j >= 0) continue;
            ...
            for (Direction direction : Direction.values()) {
                BlockPos blockPos2 = blockPos.offset(direction);
                BlockState blockState = world.getBlockState(blockPos2);
                if (!blockState.isOf(ModBlocks.BLOCK_ONE)) continue;
