...
import net.romeosnowblitz.hmh2.entity.IceCreamMakerBlockEntity;
...
public class IceCreamMakerBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof IceCreamMakerBlockEntity) {
                ItemScatterer.spawn(world, pos, (IceCreamMakerBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.ICE_CREAM_MAKER, IceCreamMakerBlockEntity::tick);
    }
}
