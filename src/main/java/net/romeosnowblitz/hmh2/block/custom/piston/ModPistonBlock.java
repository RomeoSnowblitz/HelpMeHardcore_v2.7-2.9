...
public class ModPistonBlock extends FacingBlock {
    ...
    private void tryMove(World world, BlockPos pos, BlockState state) {
        ...
        if (bl && !state.get(EXTENDED).booleanValue()) {
            ...
            int i = 1;
            if (blockState.isOf(ModBlocks.MOD_MOVING_PISTON) && blockState.get(FACING) == direction && (blockEntity = world.getBlockEntity(blockPos)) instanceof ModPistonBlockEntity && (pistonBlockEntity = (ModPistonBlockEntity)blockEntity).isExtending() && (pistonBlockEntity.getProgress(0.0f) < 0.5f || world.getTime() == pistonBlockEntity.getSavedWorldTime() || ((ServerWorld)world).isInBlockTick())) {
                i = 2;
            }
    ...
    public boolean onSyncedBlockEvent(BlockState state, World world, BlockPos pos, int type, int data) {
        ...
        if (type == 0) {
            ...
            if (this.gluey) {
                ...
                boolean bl2 = false;
                if (blockState2.isOf(ModBlocks.MOD_MOVING_PISTON) && (blockEntity2 = world.getBlockEntity(blockPos)) instanceof ModPistonBlockEntity && (pistonBlockEntity = (ModPistonBlockEntity)blockEntity2).getFacing() == direction && pistonBlockEntity.isExtending()) {
                    pistonBlockEntity.finish();
                    bl2 = true;
                }
                if (!bl2) {
                    if (type == 1 && !blockState2.isAir() && ModPistonBlock.isMovable(blockState2, world, blockPos, direction.getOpposite(), false, direction) && (blockState2.getPistonBehavior() == PistonBehavior.NORMAL || blockState2.isOf(Blocks.PISTON) || blockState2.isOf(Blocks.STICKY_PISTON) || blockState2.isOf(ModBlocks.GLUEY_PISTON))) {
    ...
    public static boolean isMovable(BlockState state, World world, BlockPos pos, Direction direction, boolean canBreak, Direction pistonDir) {
        ...
        if (state.isOf(Blocks.PISTON) || state.isOf(Blocks.STICKY_PISTON) || state.isOf(ModBlocks.GLUEY_PISTON)) {
            if (state.get(EXTENDED).booleanValue()) {
                return false;
            }
        } else {
    ...
    private boolean move(World world, BlockPos pos, Direction dir, boolean retract) {
        ...
        if (!retract && world.getBlockState(blockPos).isOf(ModBlocks.MOD_PISTON_HEAD)) {
        ...
        for (k = list3.size() - 1; k >= 0; --k) {
            ...
            ModPistonBlock.dropStacks(blockState, world, blockPos3, blockEntity);
        ...
        for (k = list.size() - 1; k >= 0; --k) {
            ...
            BlockState blockState3 = (BlockState)ModBlocks.MOD_MOVING_PISTON.getDefaultState().with(FACING, dir);
            world.setBlockState(blockPos3, blockState3, Block.NO_REDRAW | Block.MOVED);
            world.addBlockEntity(ModPistonExtensionBlock.createBlockEntityPiston(blockPos3, blockState3, (BlockState)list2.get(k), dir, retract, false));
            blockStates[j++] = blockState;
        }
        if (retract) {
            ModPistonType pistonType = this.gluey ? ModPistonType.GLUEY : ModPistonType.DEFAULT;
            BlockState blockState4 = (BlockState)((BlockState)ModBlocks.MOD_PISTON_HEAD.getDefaultState().with(ModPistonHeadBlock.FACING, dir)).with(ModPistonHeadBlock.TYPE, pistonType);
            BlockState blockState = (BlockState)((BlockState)ModBlocks.MOD_MOVING_PISTON.getDefaultState().with(ModPistonExtensionBlock.FACING, dir)).with(ModPistonExtensionBlock.TYPE, this.gluey ? ModPistonType.GLUEY : ModPistonType.DEFAULT);
            map.remove(blockPos);
            world.setBlockState(blockPos, blockState, Block.NO_REDRAW | Block.MOVED);
            world.addBlockEntity(ModPistonExtensionBlock.createBlockEntityPiston(blockPos, blockState, blockState4, dir, true, true));
        }
        ...
        if (retract) {
            world.updateNeighborsAlways(blockPos, ModBlocks.MOD_PISTON_HEAD);
        }
        return true;
    }
