...
public class AmberDust extends Block {
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
        ...
        if (wireConnection.isConnected() == ((WireConnection)state.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction))).isConnected() && !AmberDust.isFullyConnected(state)) {
    ...
    private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean bl) {
        ...
        if (bl && (bl2 = this.canRunOnTop(world, blockPos, blockState)) && AmberDust.connectsTo(world.getBlockState(blockPos.up()))) {
            if (blockState.isSideSolidFullSquare(world, blockPos, direction.getOpposite())) {
                return WireConnection.UP;
            }
            return WireConnection.SIDE;
        }
        if (AmberDust.connectsTo(blockState, direction) || !blockState.isSolidBlock(world, blockPos) && AmberDust.connectsTo(world.getBlockState(blockPos.down()))) {
            return WireConnection.SIDE;
        }
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, BlockPos sourcePos, boolean notify) {
        ...
        if (state.canPlaceAt(world, pos)) {
            ...
            AmberDust.dropStacks(state, world, pos);
    ...
    protected static boolean connectsTo(BlockState state) {
        return AmberDust.connectsTo(state, null);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (AmberDust.isFullyConnected(state) || AmberDust.isNotConnected(state)) {
            BlockState blockState = AmberDust.isFullyConnected(state) ? this.getDefaultState() : this.dotState;
