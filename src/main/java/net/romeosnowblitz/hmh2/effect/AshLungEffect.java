...
public class AshLungEffect extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        if(entity.hasStatusEffect(CustomEffects.ASH_LUNG) && entity.hasStatusEffect(CustomEffects.NORMIE) && entity.getHealth() < 2){
            entity.removeStatusEffect(this);
            entity.addStatusEffect(new StatusEffectInstance(CustomEffects.NECROMANCER,  -1,  0, false, false, true));
            int x = entity.getStatusEffect(CustomEffects.SAVING_GRACE).getAmplifier();entity.heal(x);
