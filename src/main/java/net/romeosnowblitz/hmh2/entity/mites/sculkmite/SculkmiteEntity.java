...
public class SculkmiteEntity extends HostileEntity {
    final net.minecraft.entity.AnimationState idleAnimationState = new net.minecraft.entity.AnimationState();
    ...
    public boolean tryAttack(Entity target) {
        if (getTarget() != null) {
            int i = 1;
            if (getTarget().getStatusEffect(CustomEffects.SIGHTLESSSNESS) != null) {
                i += getTarget().getStatusEffect(CustomEffects.SIGHTLESSSNESS).getAmplifier();
                getTarget().removeStatusEffectInternal(CustomEffects.SIGHTLESSSNESS);
            } else {
                --i;
            }
            i = MathHelper.clamp(i, 0, 20);
            getTarget().addStatusEffect(new StatusEffectInstance(CustomEffects.SIGHTLESSSNESS, 72000, i, false, false, true));
        }
        return target.damage(getDamageSources().create(ModDamageTypes.KILLED_BY_SCULKMITE, this), (float) this.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE));
    }
