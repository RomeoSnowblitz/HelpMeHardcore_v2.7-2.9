...
public class BloodmiteEntity extends HostileEntity {
    ...
    public boolean tryAttack(Entity target) {
        if (getTarget() != null) {
            int i = 1;
            if (getTarget().getStatusEffect(CustomEffects.BLOOD_FEVER) != null) {
                i += getTarget().getStatusEffect(CustomEffects.BLOOD_FEVER).getAmplifier();
                getTarget().removeStatusEffectInternal(CustomEffects.BLOOD_FEVER);
            } else {--i;}i = MathHelper.clamp(i, 0, 20);
            getTarget().addStatusEffect(new StatusEffectInstance(CustomEffects.BLOOD_FEVER, 72000, i, false, false, true));
        }
        return target.damage(getDamageSources().create(ModDamageTypes.KILLED_BY_BLOODMITE, this), (float) this.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE));
    }
