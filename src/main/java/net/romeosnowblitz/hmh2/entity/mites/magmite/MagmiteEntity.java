...
public class MagmiteEntity extends HostileEntity {
    ...
    public boolean tryAttack(Entity target) {
        if (getTarget() != null) {
            int i = 1;
            if (getTarget().getStatusEffect(CustomEffects.KINDLING) != null) {
                i += getTarget().getStatusEffect(CustomEffects.KINDLING).getAmplifier();
                getTarget().removeStatusEffectInternal(CustomEffects.KINDLING);
            } else {
                --i;
            }
            i = MathHelper.clamp(i, 0, 20);
            getTarget().addStatusEffect(new StatusEffectInstance(CustomEffects.KINDLING, 72000, i, false, false, true));
        }
        return target.damage(getDamageSources().create(ModDamageTypes.KILLED_BY_MAGMITE, this), (float) this.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE));
    }
