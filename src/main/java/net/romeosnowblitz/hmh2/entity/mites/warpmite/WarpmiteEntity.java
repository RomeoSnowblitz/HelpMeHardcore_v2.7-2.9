...
public class WarpmiteEntity extends HostileEntity {
    ...
    public boolean tryAttack(Entity target) {
        if (getTarget() != null) {
            int i = 1;
            if (getTarget().getStatusEffect(CustomEffects.ENDER_SKIN) != null) {
                i += getTarget().getStatusEffect(CustomEffects.ENDER_SKIN).getAmplifier();
                getTarget().removeStatusEffectInternal(CustomEffects.ENDER_SKIN);
            } else {
                --i;
            }
            i = MathHelper.clamp(i, 0, 20);
            getTarget().addStatusEffect(new StatusEffectInstance(CustomEffects.ENDER_SKIN, 72000, i, false, false, true));
        }
        return target.damage(getDamageSources().create(ModDamageTypes.KILLED_BY_WARPMITE, this), (float) this.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE));
    }
